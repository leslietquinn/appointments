<?php
	...
	final class QAppointments_Facade_Form_Fields extends QDao implements QAcceptee_Interface {
		 ...
		 *
		 * @see					QAppointments_Facade::formFields();
		 *
		 * @access				public
		 * @introduced			2022-01-16 [build date]
		 * @return				void
		...
		public function push( $acceptable ) : void {
			if( !( $acceptable instanceof QDataspace_Interface or $acceptable instanceof QPage_Interface ) ) {
				throw new QPage_Exception( 'thrown exception: unsupported interface [./appointments/facade/form/fields] 36' );
			}
			
			$record = new QAppointments_Record( $acceptable -> get( 'id' ) );
			
			if( !$record -> has( 'id' ) ) {
				throw new QPage_Exception( 'thrown exception: invalid primary key given [./appointments/facade/form/fields] 42' );
			}
