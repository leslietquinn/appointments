<?php
	...
	final class QAppointments_Facade_Remove extends QDao implements QAcceptee_Interface {
		 ...
		 *
		 * @see				QAppointments_Facade::remove();
		 *
		 * @access			public
		 * @introduced		2022-01-16 [build date]
		 * @return			bool
		...
		public function push( $acceptable ) : bool {
			if( !( $acceptable instanceof QDataspace_Interface or $acceptable instanceof QPage_Interface ) ) {
				throw new QPage_Exception( 'thrown exception: unsupported interface [./appointments/facade/remove] 30' );
			}
			
			$record = new QAppointments_Record( $acceptable -> get( 'id' ) );
			
			if( !$record -> has( 'id' ) ) {
				throw new QPage_Exception( 'thrown exception: invalid primary key given [./appointments/facade/remove] 36' );
			}
