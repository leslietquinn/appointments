<?php
	...
	final class QUsers_Facade_Load extends QDao implements QAcceptee_Interface {
		...
		public function push( $acceptable ) : void {
			if( !( $acceptable instanceof QDataspace_Interface or $acceptable instanceof QPage_Interface ) ) {
				throw new QPage_Exception( 'thrown exception: unsupported interface [./users/facade/load] 37' );
			}
			
			$record = new QUsers_Record( $acceptable -> get( 'id' ) );
			
			if( !$record -> has( 'id' ) ) {
				throw new QPage_Exception( 'thrown exception: invalid primary key given [./users/facade/load] 43' );
			}
