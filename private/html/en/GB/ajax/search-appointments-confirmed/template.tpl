					...
					<?php if( $this -> has( 'confirmed' ) ): ?>
					<select name="dashboard" id="confirmed" size="<?php echo QAppointments::MENU_SIZE; ?>" class="corners">
						<option value="0" selected="selected">- Select a Record -</option>
						
						<?php foreach( $this -> get( 'confirmed' ) as $option ): ?>
